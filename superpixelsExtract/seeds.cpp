...
void SEEDS::deinitialize() 
{
  if(initialized) 
  {
    initialized = false;
		...
		{
            for (unsigned int label=0; label<nr_labels[level]; label++)
			{
...
void SEEDS::initialize(int seeds_w, int seeds_h, int nr_levels)
{
  deinitialize();
	...
	// init labels
  labels = new unsigned int*[seeds_nr_levels];
  parent = new unsigned int*[seeds_nr_levels];
  nr_partitions = new unsigned int*[seeds_nr_levels];
  nr_labels = new unsigned int[seeds_nr_levels];
  ...
  labels[level] = new unsigned int[width*height];
  parent[level] = new unsigned int[nr_seeds];
  nr_partitions[level] = new unsigned int[nr_seeds];
...
void SEEDS::update_image_ycbcr(unsigned int* image) {
...
void SEEDS::compute_means()
{
    ...
    for (unsigned int label=0; label<nr_labels[seeds_top_level]; label++)
	{
		L_channel[label] = 0;
		A_channel[label] = 0;
		B_channel[label] = 0;
...
void SEEDS::compute_histograms(int until_level)
{
	...
	{
        for (unsigned int label=0; label<nr_labels[level-1]; label++)
		{
...
void SEEDS::LAB2RGB(float L, float a, float b, int* R, int* G, int* B)
{
    ...
    float fY = pow((L + 16.0f) / 116.0f, 3);
	bool YT = fY > T1;
    fY = (!YT) * (L / 903.3f) + YT * fY;
	float Y = fY;

	// Alter fY slightly for further calculations
    fY = YT * pow(fY,1.0f/3.0f) + (!YT) * (7.787f * fY + 16.0f/116.0f);

    float fX = a / 500.0f + fY;
	bool XT = fX > T2;
    float X = (XT * pow(fX, 3) + (!XT) * ((fX - 16.0f/116.0f) / 7.787f));

    float fZ = fY - b / 200.0f;
	bool ZT = fZ > T2;
    float Z = (ZT * pow(fZ, 3) + (!ZT) * ((fZ - 16.0f/116.0f) / 7.787f));

    X = X * 0.950456f * 255.0f;
    Y = Y * 255.0f;
    Z = Z * 1.088754f * 255.0f;

    *R = (int) (3.240479f*X - 1.537150f*Y - 0.498535f*Z);
    *G = (int) (-0.969256f*X + 1.875992f*Y + 0.041556f*Z);
    *B = (int) (0.055648f*X - 0.204043f*Y + 1.057311f*Z);
}
...
int SEEDS::RGB2LAB(const int& r, const int& g, const int& b, float* lval, float* aval, float* bval)
{
    ...
    fX = XT * pow(xVal,1.0f/3.0f) + (!XT) * (7.787f * xVal + 16.0f/116.0f);

	// Compute L
    float Y3 = pow(yVal,1.0f/3.0f);
    fY = YT*Y3 + (!YT)*(7.787f*yVal + 16.0f/116.0f);
    lVal  = YT * (116 * Y3 - 16.0f) + (!YT)*(903.3f*yVal);

    fZ = ZT*pow(zVal,1.0f/3.0f) + (!ZT)*(7.787f*zVal + 16.0f/116.0f);
...
int SEEDS::RGB2HSV(const int& r, const int& g, const int& b, float* hval, float* sval, float* vval)
{
    ...
    float min_rgb = std::min(r_, std::min(g_, b_));
...
{
	// binning
    int bin_l = (int)floor(l/100.1f*nr_bins);

    int bin_a = (int)floor(a/100*(nr_bins-2) + (nr_bins-2)/2 + 1);
	if (bin_a < 0) bin_a = 0;
	if (bin_a >= nr_bins) bin_a = nr_bins-1;

    int bin_b = (int)floor(b/100*(nr_bins-2) + (nr_bins-2)/2 + 1);
	if (bin_b < 0) bin_b = 0;
...
int SEEDS::RGB2LAB_special(int r, int g, int b, float* lval, float* aval, float* bval)
{
    ...
    fX = XT * pow(xVal,1.0f/3.0f) + (!XT) * (7.787f * xVal + 16/116);

	// Compute L
    float Y3 = pow(yVal,1.0f/3.0f);
    fY = YT*Y3 + (!YT)*(7.787f*yVal + 16/116);
    lVal  = YT * (116 * Y3 - 16.0f) + (!YT)*(903.3f*yVal);

    fZ = ZT*pow(zVal,1.0f/3.0f) + (!ZT)*(7.787f*zVal + 16/116);
...
int SEEDS::threebythree(int x, int y, unsigned int label)
{
	int count = 0;
    const unsigned int* ptr_label = labels[seeds_top_level];
	const int y_width = y*width;
...
int SEEDS::threebyfour(int x, int y, unsigned int label)
{
	int count = 0;
    const unsigned int* ptr_label = labels[seeds_top_level];
	const int y_width = y*width;
...
int SEEDS::fourbythree(int x, int y, unsigned int label)
{
	int count = 0;
    const unsigned int* ptr_label = labels[seeds_top_level];
	const int y_width = y*width;
...
void SEEDS::SaveLabels_Text(std::string filename)
{
        ...
        for( int h = 0; h < height; h++ )
          {
            ...
            outfile << labels[seeds_top_level][i] << std::endl;
            i++;
          }
	outfile.close();
}
...
{
    means = new unsigned int[width*height];

	for (int i=0; i<width*height; i++)
	{
		int label = labels[seeds_top_level][i];
        float L = 100.0f * ((float) L_channel[label]) / T[seeds_top_level][label];
        float a = 255.0f * ((float) A_channel[label]) / T[seeds_top_level][label] - 128.0f;
        float b = 255.0f * ((float) B_channel[label]) / T[seeds_top_level][label] - 128.0f;
		int R, G, B;
		LAB2RGB(L, a, b, &R, &G, &B);
		means[i] = B | (G << 8) | (R << 16);
	}
